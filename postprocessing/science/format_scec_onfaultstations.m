...
for i = 1:nr_stations
    ...
    fprintf(fid,'# code=SeisSol (ADER-DG) without plasticity\n');
...
%    fprintf(fid,'# Background stress is assigned to a complete element determined by its geometrical barycenter\n');
    fprintf(fid,'# element_size=between 12.5 m and 200m on fault (depending on gradient)');
    fprintf(fid,'# 500m in a 5km width zone surrounding the fault, 5km far away, Growth rate 1.6\n');
    fprintf(fid,'# 8,4e6 tetra elements\n');
